...
PHOTO_ANALYSIS_PROMPT = """Проанализируй фото. Это может быть:
- ЕДА
- ФИТНЕС-ТРЕКЕР (Apple Watch, Mi Band, Samsung Health)
- МЕДИЦИНСКИЕ АНАЛИЗЫ (кровь, биохимия, моча и т.д.)
...
===== ЕСЛИ ЭТО МЕДИЦИНСКИЕ АНАЛИЗЫ =====
{
    "type": "medical",
    "analysis_type": "кровь" | "биохимия" | "моча" | "гормоны" | "другое",
    "date": "дата анализа если видно (ДД.ММ.ГГГГ)",
    "indicators": [
        {
            "name": "название показателя на русском",
            "value": число или строка,
            "unit": "единица измерения",
            "reference_range": "норма (например: 4.0-6.0)",
            "status": "normal" | "high" | "low"
        }
    ],
    "summary": "Краткое описание результатов",
    "nutrition_recommendations": [
        "рекомендация по питанию 1",
        "рекомендация по питанию 2"
    ],
    "concerns": ["проблема 1 если есть", "проблема 2"]
}
...
- Для анализов: извлеки ВСЕ показатели, определи статус (норма/повышен/понижен), дай рекомендации по питанию
